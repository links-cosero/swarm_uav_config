...
class STC_handler
{
  ...
  //numcolsreturns a neighbour in the specified direction to the cell
...
class DARP
{
    ...
    Eigen::Matrix<double,6,6> Eval_mat1;
    Eigen::Matrix<double,6,6> Eval_mat2;
    Eigen::Matrix<double,6,6> Assign_mat;
    
    Eigen::Matrix<int,2,2> in_pos;
    int k1, k2;
    double m1,m2;
    double f;
    ...
    // 6x6 grid map
    DARP(int numrows, int numcols, int numDrones, Eigen::Matrix<int,2,2> initial_pos)
    {
        rows = numrows;
        cols = numcols;
        nr = numDrones;
    ...
    /**
     * For every cell calculates the distance from each robot
    */
    void calculateEvaluation_matrix()
    {
      ...
      {
          int drone_x = in_pos(count, 0);
          int drone_y = in_pos(count, 1);

          // cout << "Drone " << count << " x = " << drone_x << " y = " << drone_y << endl;
          
          // Euclidean distance ( 21- 15, not so good )

          // for (i=0; i<rows; i++)
          // {
          //     for (j=0; j<cols; j++)
          //     {
          //         dist = sqrt(pow(drone_x - i, 2) + pow(drone_y - j, 2));
          //         if (count == 0) 
          //           Eval_mat1(i,j) = dist;
          //         else
          //           Eval_mat2(i,j) = dist;
          ...
          //   for(j=0;j<cols;j++)
          //   {
          //     dist = abs(drone_x - i) + abs(drone_y -j);
          ...
          // Chebyshev Distance ( same )
          cout << "Chebyshev Distance\n" << endl;    
          for (i=0;i<rows;i++)
          {
            for(j=0;j<cols;j++)
            {
              dist = max(abs(drone_x - i), abs(drone_y -j));
    ...
    void calculateAssignment_matrix()
    {
        k1 = 0;
        k2 = 0;
        ...
        for(int i=0; i<rows; i++)
        {
          ...
          {
              if(Eval_mat1(i,j) <= Eval_mat2(i,j))
              {
                  //Assign the (i,j) cell to the first drone
                  Assign_mat(i,j) = 1;
                  k1++;
                  // cout << k1 << endl;
              }else
              {
                  //Otherwise, assign the (i,j) cell to the second drone
                  Assign_mat(i,j) = 2;
                  k2++;
              }                  
          }
    ...
    /*
    * Calculate the m factor to update evaluation matrix E
    */
    void calculateDiscrepancy()
    {
        ...
        int threshold = 1, result, iteration = 0;
        double c = 0.33;

        f = (rows*cols)/nr; // Fair Share calculation

        cout << "Fair Share: " << f << " Tuning Factor: " << c << endl;
        ...
        do
        {
            ...
            cout << "iteration = " << iteration << " result = "<< result << endl;
        ...
        }while (result >= threshold && iteration <= 5);
    }
  
    void showEvaluation_matrix1()
    { 
        
        cout << "Evaluation Matrix 1\n" << Eval_mat1 << endl;

    }

    void showEvaluation_matrix2()
    { 
        
        cout << "Evaluation Matrix 2\n" << Eval_mat2 << endl;

    }

    void showAssignment_matrix()
    { 
    ...
    void getNumCellAssigned1()
    {
      cout << "Numero celle assegnato a drone 1 = " << k1 << endl;
    }

    void getNumCellAssigned2()
    {
      cout << "Numero celle assegnato a drone 2 = " << k2 << endl;
    }
...
void OffboardControl::mission_cb()
{
    ...
    }else if (mission_state == 1)
    {
      ...
      // if the drone is below 5 cm range then compute next waypoint 
        if(abs(drone_position[0] - current_waypoint[0]) <= 0.15 &&
            abs(drone_position[1] - current_waypoint[1]) <= 0.15 &&
            abs(drone_position[2] - current_waypoint[2]) <= 0.15) 
        {
            subcell = *trajectory_subcells_iterator;
            Eigen::Matrix<double,3,1> c_w = {subcell->getSubCell_x_drone(), 
            subcell->getSubCell_y_drone(), 0.5};
...
int main(int argc, char *argv[])
{
  ...
  // 6. DARP: divisone delle aree
  int droneNum = 2; // 2 droni
  Eigen::Matrix2i initial_pos {{0,0},{5,5}}; //posizioni iniziali dei due droni
  int dim_x = 6, dim_y = 6; // 6x6 Grid

  DARP division(dim_x,dim_y,droneNum,initial_pos);
  division.calculateEvaluation_matrix();
  division.showEvaluation_matrix1();
  division.showEvaluation_matrix2();

  division.calculateAssignment_matrix();
  division.showAssignment_matrix();
  division.getNumCellAssigned1();
  division.getNumCellAssigned2();

  division.calculateDiscrepancy();
